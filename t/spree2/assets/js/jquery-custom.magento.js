...
(function()
{
    ...
    if (anchor_cart.length > 0)
    {
        ...
        anchor_cart.on('click touchstart', function(e)
        {
			e.preventDefault(); // Prevent # in address bar
            if(this.rel>0){
				onAddToCartLinkClick(this, this.rel);
			}else{
				onAddToCartLinkClick(this);
			}
					
            if (isAddInCartInProgress())
            {
                return false;
            }
            setAddInCartInProgress(true);
    ...
    {
        var cartLoaderImage = websiteURL + assetsFolder + 'images/cart-loader.gif';
        cart_counter.html("<img src='"+ cartLoaderImage +"' align='top' />");
    ...
    var address_suggestions = $j('#modal'); // Modal
    var current_form = '';
    // Address form input
    var input_street_1 = null;
    var input_street_2 = null;
    var input_suburb = null;
    var input_city = null;
    var input_postal_code =null;
    ...
    $j('[data-suggest-address="true"]').each(function()
    {
        ...
        if (address_suggestions.length > 0)
        {
            ...
            $j(this).keyup(function()
            {
                ...
                current_form = $j(this).closest('form');

                var prefix = '';
                var objectId = (this.id).split(':');
                if (objectId.length > 1)
                {// elements are billing/shipping postcodes
                    prefix = objectId[0] + ":";
                    $j('#field_prefix').val(prefix);
                }

                // Address form input
                input_street_1 = $j('[data-address-field="' + prefix + 'street-1"]', current_form);
                input_street_2 = $j('[data-address-field="' + prefix + 'street-2"]', current_form);
                input_suburb = $j('[data-address-field="' + prefix + 'suburb"]', current_form);
                input_city = $j('[data-address-field="' + prefix + 'city"]', current_form);
                input_postal_code = $j('[data-address-field="' + prefix + 'postal-code"]', current_form);
                //checkout and address management
                input_country = $j('[name="'+ prefix.replace(":", "") +'[country_id]"]', current_form);
                if(!input_country.val())input_country = $j('[name="country_id"]', current_form);
				
                
                // Enforce the user to have all these field filled in before proceeding
                if ($j.trim(input_suburb.val()).length > 0 &&
                    $j.trim(input_city.val()).length > 0 &&
                    input_country.val() == "ZA")
                {

                    look_up_url = input_postal_code.attr("data-url");

                    // Number of characters entered meets threshold
                    if ($j(this).val().length >= threshold)
                    {
                        // Launch modal window
                        suggestAddresses(look_up_url);
                    }
                }
            });
        }
     ...
     */
    function suggestAddresses(look_up_url)
    {
        $j('#current_form').val(current_form.attr("id"));
        $j(".modal-inner-content").html(loadingIcon);

        // Launch modal window
        address_suggestions.modal(options_modal);

        var params = {
            'street_line_1' : input_street_1.val(),
            'street_line_2' : input_street_2.val(),
            'suburb' : input_suburb.val(),
            'city' : input_city.val(),
            'country': input_country.val(),
			'postcode': input_postal_code.val(),
            'current_for' : input_city.val()
        };

        $j.get(look_up_url, params, function(html){
            $j(".modal-inner-content").html(html);
        },'html');
